...
A stronger anchor exists and is not January work: a trusted timestamp over `season-2026-manifest.txt` puts the set in the Bitcoin chain and removes GitHub from the trusted set entirely. The procedure, what leaves the machine, and why it expires on 22 August rather than at the end of the season are in [TIMESTAMP.md](TIMESTAMP.md). Until it lands, the honest description of this registry is that its integrity is cryptographic and its ordering is only as good as a third party's log.
